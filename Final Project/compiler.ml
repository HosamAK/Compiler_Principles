...
module Reader : READER = struct
  ...
  let pack_parse3 arg1 arg2 arg3 =
    let res = caten arg1 arg2 in
    pack res arg3;;
  ...
  and nt_vector str =
    let nt_par_open = word "#(" in
    let nt_cat1 = caten nt_skip_star nt_par_open in
    let nt_par_close = char ')' in
    let nt_pack_res = pack nt_par_close (fun _ -> ScmVector []) in
    let nt_cat_res = caten (plus nt_sexpr) nt_par_close in
    let nt_cat_pack_res = pack nt_cat_res (fun (sexprs, _) -> ScmVector sexprs) in
    let nt_disj_res = disj nt_pack_res nt_cat_pack_res in

    let nt_final = pack_parse3 nt_cat1 nt_disj_res (fun (_, sexpr) -> sexpr) in
    nt_final str


  and nt_list str =
    let nt_par_open = char '(' in
    let nt_par_close = char ')' in
    let nt_cat_res = caten nt_skip_star nt_par_close in
    let nt_cat_pack_res = pack nt_cat_res (fun _ -> ScmNil) in
    let ntProper = pack nt_par_close (fun _ -> ScmNil) in
    let ntImproper = pack (caten (char '.') (caten nt_sexpr nt_par_close))
                                          (fun ( _, (sexpr,_)) -> sexpr) in
    let nt_disj_res = disj ntProper ntImproper in
    let anno_func=(fun (sexprs, sexpr) -> 
      List.fold_right
       (fun car cdr -> ScmPair (car, cdr))
      sexprs
      sexpr) in
    let pack_res = pack (caten (plus nt_sexpr) nt_disj_res) anno_func in

    let disj_res = disj nt_cat_pack_res pack_res in
    let nt_final = pack (caten nt_par_open disj_res) (fun ( _,sexpr) -> sexpr) in
    make_skipped_star nt_final str 
...
let read str = (Reader.nt_sexpr str 0).found;;
 ...
 let rec scmlst_tonarmallst = function
 | ScmPair (car, cdr) -> car :: (scmlst_tonarmallst cdr)
 | ScmNil -> []
 | sexpr -> raise (X_syntax "Not a proper list")
;;

let rec normallst_toscmlst = function
 | [] -> ScmNil
 | car :: [] -> ScmPair (car, ScmNil)
 | car :: cdr -> ScmPair (car, normallst_toscmlst cdr)
;;
...
let get_cdr list = 
 match list with
   | ScmPair(car, ScmPair (cdr, ScmNil)) -> cdr
   | _ -> ScmNil
;;
...
let check_cars sexpr = 
  (normallst_toscmlst (our_map get_car (scmlst_tonarmallst sexpr)));;

let check_cdrs sexpr = 
  (normallst_toscmlst (our_map get_cdr (scmlst_tonarmallst sexpr)));;
   
let rec scheme_map f sexpr =
 match sexpr with
   | ScmNil -> ScmNil
   | ScmPair (car, cdr) -> ScmPair (f car, scheme_map f cdr)
   | _ -> raise (X_syntax "Expected proper list but got not proper list")
;;

let add_setbang sexp1 sexp2 = 
 ScmPair(ScmSymbol "set!", ScmPair(sexp1, ScmPair(sexp2, ScmNil)));;


let no_matter_pair =
  ScmPair(ScmSymbol "no_matter", ScmNil)


let quote_pair =
  ScmPair(ScmPair(ScmSymbol "quote", no_matter_pair), ScmNil)


let two_pairs sexp = 
 ScmPair(sexp, quote_pair);;


let rec combine_lists_with_func func se1 se2 =
 match se1, se2 with
   | ScmNil, ScmNil -> ScmNil
   | ScmPair (car1, cdr1), ScmPair (car2, cdr2) -> 
       ScmPair (func car1 car2, combine_lists_with_func func cdr1 cdr2)
   | _, _ ->
       raise (X_syntax "Expected same length but got defferent legnth")
;;
     

let mylet_scmpair ribs =
  ScmPair(
                            (scheme_map two_pairs (check_cars ribs)),
                            combine_lists_with_func add_setbang (check_cars ribs) 
                            (check_cdrs ribs))



let rec append_exprs_to_list list exprs =
 match list with
   | ScmNil -> exprs
   | ScmPair (car, cdr) -> ScmPair (car, append_exprs_to_list cdr exprs)
   | _ -> raise (X_syntax "Expects a proper list")
;;
...
module Tag_Parser : TAG_PARSER = struct
  ...
  let rec macro_expand_qq = function
    ...
    | ScmPair (car, cdr) ->
      ScmPair
        (ScmSymbol "cons",
         ScmPair ( macro_expand_qq car, ScmPair (macro_expand_qq cdr, ScmNil)))
    | ScmVector sexprs ->
  ...
  (* functions for cond expantions *)
  let empty = ScmVoid;;

  let one_else exprs = ScmPair (ScmSymbol "begin", exprs);;

  let symif = ScmSymbol "if";;

  let symbeg = ScmSymbol "begin";;

  let pair_begin_expers symbeg exprs = ScmPair (symbeg, exprs);;

  let big_pair symif pred pair_begin_expers symbeg exprs what_left = ScmPair (symif,
                                                                      ScmPair (pred,
                                                                        ScmPair
                                                                          (pair_begin_expers symbeg exprs,
                                                                            ScmPair (what_left, ScmNil))));;


  let lamb_left what_left= 
    ScmPair
                           (ScmPair
                              (ScmSymbol "lambda",
                               ScmPair (ScmNil, ScmPair (what_left, ScmNil))),
                            ScmNil);;
  ...
  let mid_mid_pair expr func what_left = 
        ScmPair
            (ScmPair
             ...
             ScmPair
               (ScmPair
                  (ScmSymbol "if",
                    val_pair),
                ScmNil));;
  let mid_pair what_left expr func = 
    ScmPair
         (ScmSymbol "let",
          mid_mid_pair expr func what_left);;
  (* ---------- END OF FUNCTIONS ----------------*)
  ...
  let rec macro_expand_cond_ribs ribs = 
    ...
    | ScmPair (ScmPair (expr,
                        ScmPair (ScmSymbol "=>",
                                 ScmPair (func, ScmNil))),
               ribs) ->
       let what_left = macro_expand_cond_ribs ribs in
       mid_pair what_left expr func

    | ScmPair (ScmPair (pred, exprs), ribs) ->
       let what_left = macro_expand_cond_ribs ribs in
       big_pair symif pred pair_begin_expers symbeg exprs what_left
    | _ -> raise (X_syntax "incorrect cond-rib");;;;
  ...
  let rec tag_parse sexpr =
    ...
    | ScmPair (ScmSymbol "letrec", ScmPair (ribs, exprs)) ->
      (match ribs with 
        ...
        |_ -> tag_parse (append_exprs_to_list 
                          (ScmPair (ScmSymbol "let", mylet_scmpair ribs))
                           exprs))
...
module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  ...
  let lexical_scmvarget str params env= 
    ScmVarGet'(tag_lexical_address_for_var str params env)

  let lexical_scmif test dif dit =
    ScmIf'(test , dif, dit);;
  
  let lexical_scmseq run params env exprs_ls =
    ScmSeq'(List.map (fun x -> run x params env) exprs_ls);;
  
  let lexical_scmor run params env exprs_ls =
    ScmOr'(List.map (fun x -> run x params env) exprs_ls);;

  let tag_lexical str params env = (tag_lexical_address_for_var str params env);;

  let lexical_scmvarset arg1 arg2 =
    ScmVarSet'(arg1 , arg2);;

  let lexical_scmvardef arg1 arg2 =
    ScmVarDef'(arg1, arg2);;

  let lexical_scmapplic arg1 arg2 arg3 =
    ScmApplic'(arg1, arg2, arg3);;
  ...
  let annotate_lexical_address pe =
    (* HW 2 implem*)
    let rec run exprs params env = 
      match exprs with
      | ScmConst(sexpr) -> ScmConst'(sexpr)
      | ScmVarGet(Var str) -> lexical_scmvarget str params env
      | ScmIf(test, dif, dit) ->
          let run_test = (run test params env) in
          let run_dif = (run dif params env) in
          let run_dit = (run dit params env) in
          lexical_scmif run_test run_dif run_dit
      | ScmSeq(exprs_ls) -> lexical_scmseq run params env exprs_ls
      | ScmOr(exprs_ls) -> lexical_scmor run params env exprs_ls
      | ScmVarSet(Var str , exp) ->
        let arg1 = (tag_lexical str params env) in
        let arg2 = (run exp params env) in
        lexical_scmvarset arg1 arg2
      | ScmVarDef(Var str, exp) -> 
        let arg1 = (tag_lexical str [] [[]]) in
        let arg2 = (run exp params env) in
        lexical_scmvardef arg1 arg2
      | ScmLambda(param_list, kind, exp) -> 
          let ans = match kind with
          | Simple -> ScmLambda'(param_list, kind, run exp param_list (params :: env))
          | Opt(someString) -> ScmLambda'(param_list, kind, (run exp (param_list @ [someString]) ( params :: env) ) ) in
           ans
      | ScmApplic(exp, exp_list) ->
        let arg1 = (run exp params env) in
        let arg2 = (List.map (fun x -> run x params env) exp_list) in
        lexical_scmapplic arg1 arg2 Non_Tail_Call in
      run pe [] [[]];;


  let tail_scmif arg1 arg2 arg3 = 
    ScmIf' (arg1, arg2, arg3);;

  let tail_scmor_help exprs_map run_tail =
    exprs_map @ [run_tail];;
  
  let tail_scmor exprs_map run_tail =
    ScmOr' (tail_scmor_help exprs_map run_tail);; 

  let tail_scmvarset arg1 arg2 = 
    ScmVarSet' (arg1, arg2);;
  
  let tail_scmvardef arg1 arg2= 
    ScmVarDef' (arg1 , arg2);;

  let tail_scmboxset arg1 arg2 = 
    ScmBoxSet' (arg1, arg2)

  let tail_scmlambda arg1 arg2 arg3 = 
    ScmLambda'(arg1, arg2, arg3) 
  
  let tail_scmapplic arg1 arg2 arg3=
    ScmApplic'(arg1, arg2, arg3) ;;
  (* run this second *)
  let annotate_tail_calls pe = 
    (* HW 2 implem*)
    let rec run in_tail_pos curr_exp= match curr_exp with
    | ScmConst' const_exp ->  ScmConst' const_exp
    | ScmVarGet' expr1 -> ScmVarGet' expr1 
    | ScmIf' (test',then',else') ->
      let arg1 = (run false test') in
      let arg2 = (run in_tail_pos then') in
      let arg3 = (run in_tail_pos else') in
      tail_scmif arg1 arg2 arg3
    | ScmOr' or_exprs ->  (match or_exprs with 
        | [] -> ScmOr' or_exprs
        | [first_or_exprs] -> ScmOr' [(run in_tail_pos first_or_exprs)]
        | many_or_exprs ->
          let last_expr= (last_only many_or_exprs) in
          let first_exprs= (all_but_last many_or_exprs) in
          let exprs_map = ( List.map (run false) first_exprs) in
          let run_tail = (run in_tail_pos last_expr) in
          
          tail_scmor exprs_map run_tail
        )
    | ScmVarSet' (var,expr) -> 
        let run_expr = (run false expr) in
        tail_scmvarset var run_expr
    |  ScmVarDef' (var , expr) -> 
        let run_expr = (run false expr) in
        tail_scmvardef var run_expr
    | ScmBox' var -> ScmBox' var
    | ScmBoxGet' var -> ScmBoxGet' var
    | ScmBoxSet' (var,expr) -> 
      let run_expr = (run false expr) in
      tail_scmboxset var run_expr 
    | ScmLambda' (params, typ, body) ->
        let run_body = (run true body) in
        tail_scmlambda params typ run_body 
    | ScmApplic' (lambda_expr, args, Non_Tail_Call) -> 
        let rec run' args = match args with 
        | [] -> []
        | arg :: [] -> ( run false arg) :: []
        | arg :: rest -> ( run false arg) :: (run' rest) in
              let taill = in_tail_pos in
              (match taill with 
              | true -> 
                let run_lambda = ( run false lambda_expr ) in
                let run_args = (run' args) in
                tail_scmapplic run_lambda run_args Tail_Call
              | false -> ScmApplic'( ( run in_tail_pos lambda_expr) , (run' args) , Non_Tail_Call) 
              )

    | ScmSeq' seq_of_exprs ->  (match seq_of_exprs with
        | [] -> ScmSeq' []
        | [expr] -> ScmSeq' [(run in_tail_pos expr)]
        | exprs_list -> 
          let last_expr= (last_only exprs_list) in
          let first_exprs= (all_but_last exprs_list) in
          ScmSeq'( (List.map (run false) first_exprs) @  [(run in_tail_pos last_expr)] )
                                ) in 
    run false pe
...
module Code_Generation : CODE_GENERATION = struct
  ...
  (*   HELPER FUNCTIONS   *)
      let gen_scmif arg1 arg2 arg3 = 
        arg1 @ arg2 @ arg3;;

      let gen_scmvarset arg1 arg2 =
        [arg1] @ arg2;;
      let gen_scmapplic arg1 arg2 =
        arg1 @ arg2;;
  ...
  let collect_free_vars pe =
    ...
    let rec run = function
      ...
      | ScmVarGet' _ -> []
      | ScmIf' (test, dit, dif) -> 
        let run_test = (run test) in
        let run_dit = (run dit) in
        let run_dif = (run dif) in
        gen_scmif run_test run_dit run_dif
      | ScmSeq' exprs' -> runs exprs'
      | ScmOr' exprs' -> runs exprs'
      | ScmVarSet' (Var' (v, Free), expr') ->
        let run_expr = (run expr') in
        gen_scmvarset v run_expr
      ...
      | ScmApplic' (expr', exprs', _) ->
        let run_expr = (run expr') in
        let runs_exprs = (runs exprs') in
        gen_scmapplic run_expr runs_exprs
    and runs exprs' =
      List.fold_left
       ...
       let primitives =
         List.map
           ...
           global_bindings_table
       and free_vars_in_code = runs pe in
       remove_duplicates
    ...
    let rec run params env exp'= 
      ...
      | ScmIf' (test, dit, dif) ->
        ...
        let asm_code = 
            ...
            let then_code = run params env dit in
            let else_code = run params env dif in
      ...
      | ScmLambda' (params', Simple, body) ->
         let label_loop_env = make_lambda_simple_loop_env () in
         let label_loop_env_end = make_lambda_simple_loop_env_end () in
         let label_loop_params = make_lambda_simple_loop_params () in
         let label_loop_params_end = make_lambda_simple_loop_params_end () in
         let label_code = make_lambda_simple_code () in
         let label_arity_ok = make_lambda_simple_arity_ok () in
         let label_end = make_lambda_simple_end () in
      ...
      | ScmLambda' (params', Opt opt, body) ->
        ...
        let label_loop_params = make_lambda_opt_loop_params() in
        let label_loop_params_end =  make_lambda_opt_loop_params_end() in
        let label_code =  make_lambda_opt_code () in
        let label_arity_ok_opt = make_lambda_opt_arity_ok () in
        let label_end = make_lambda_opt_end () in
        let label_opt_arg_empty= make_opt_arg_empty () in
        let label_build_opt_list= make_build_opt_list() in
        let label_update_built_opt_list= make_update_opt_list () in
        let label_push_up_the_stack= make_push_up_the_stack () in
        let label_push_down_the_stack= make_push_down_the_stack () in
        let label_update_arg_count_and_rsp_non_empt= make_update_rsp_opt_list_case () in
        let label_stack_fix_end= make_lambda_opt_stack_fix_end () in
        let label_push_empty_list_update_rsp= make_push_empty_list_and_update_rsp () in
        let count = (List.length params') + 1 in
           ...
           ^ (Printf.sprintf "\tmov rcx, r9 ; rcx will have the value: args_count - count\n" )
      ...
      | ScmApplic' (proc, args, Non_Tail_Call) -> 
        ...
        let asm_code = 
          ...
          let n_code = (Printf.sprintf "\tpush %d\n" num_of_args) in
          let proc_code = run params env proc in

          (match (list_and_last rev_args) with
          | Some (args, last_arg) ->
              let args_code =
                String.concat ""
                  (List.map
              ...
              let last_arg_code = run params env last_arg in
